    ...
    <div className="App">
      <header className="App-header">
        <div>
          ...
          <br />
          <a
            href={`https://ropsten.etherscan.io/address/${id}#events`}
            style={{color: 'white'}}>
            {id}
          </a>
        </div>
        <br />
        <FormControl
          style={{fontSize: 30, marginRight: 20, width: '70%'}}
          placeholder={'Amount'}
          value={depositAmount}
          onChange={w => setDepositAmount(w.target.value)}
        />
        <br />
        <Button onClick={depositMoney} style={{fontSize: 30, paddingLeft: 10}}>
          Deposit Money
        </Button>
        <br />
        <br />
        ...
        />
        <br />
        <Button onClick={sendMoney} style={{fontSize: 30, paddingLeft: 10}}>
          Send Money
        </Button>
        <br />
        <br />
...
const mapDispatchToProps = dispatch => ({
  deposit: (contract, from, amount) =>
    dispatch(actions.depositMoney(contract, from, amount)),
  send: (contract, from, to, amount) =>
    dispatch(actions.sendMoney(contract, from, to, amount)),
});
